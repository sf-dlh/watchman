...
resource "aws_instance" "watchman" {
  ...
  user_data = <<-EOF
        ...
        docker pull ${aws_ecr_repository.watchman_repo.repository_url}:0.3

        docker run -e AWS_REGION=${var.aws_region} -d --name watchman --restart always ${aws_ecr_repository.watchman_repo.repository_url}:0.3
        EOF
